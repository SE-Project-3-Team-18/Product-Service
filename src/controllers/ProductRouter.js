  ...
  try {
    ...
    if (!name || !description || !category || !sellerId) {
      throw new CustomError(
        400,
        "Name, Description , Category and Seller are required"
      );
    }
    await product.save();
    response.status(200).json(product);
...
ProductRouter.put("/buy", async (request, response, next) => {
  try {
    ...
    console.log("All items succesfully purchased in Product Router in Product Service")
    response.status(200).json({ message: "Products bought successfully" });
  } catch (error) {
    next(error);
  }
});
...
ProductRouter.put("/refund", async (request, response, next) => {
  try {
    ...
    console.log("All items succesfully refunded in Product Router in Product Service")
    response.status(200).json({ message: "Products bought successfully" });
  } catch (error) {
    next(error);
  }
